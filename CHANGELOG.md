...
Screenshots no longer capture the content of blocked elements. An element marked `rr-block` or `gl-block`, or matching the `blockClass` / `blockSelector` replay options, was copied into the page snapshot in full (text, attributes, form fields, images, web component content) and only hidden when the screenshot was rendered. It is now replaced by an empty element of the same tag and size that keeps only its class and id, the way replays record it; images inside it are not downloaded. The screenshot renders it blank, which now also holds for blocked elements inside web components and for `blockClass` / `blockSelector` elements, whose content used to show.
Text inside elements marked `rr-mask` or `gl-mask`, or matching the `maskTextClass` / `maskTextSelector` replay options, is now masked in screenshots as in replays: every character except whitespace becomes `*`, or the site's `maskTextFn` formats it. Comments inside these elements are left out. `maskTextFn` is typed `(text, element) => string`, as rrweb calls it.
